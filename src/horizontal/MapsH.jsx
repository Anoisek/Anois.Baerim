...
// marker CRUD, progress import/export) is copied verbatim from Maps.jsx —
// this map is the site's most irreplaceable data, so only className/colors
// and the chrome (NavbarH instead of Navbar) change in this file.
    ...
    <div className="min-h-screen bg-[#14110d] text-white">
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
          <h1 className="text-xl font-bold text-gray-100">{t('maps.title')}</h1>
          <div className="flex items-center gap-2 flex-wrap">
            <PillButton onClick={handleExportProgress} title={t('maps.exportProgressTooltip')}>{t('maps.exportProgress')}</PillButton>
            <PillButton onClick={handleImportProgressClick} title={t('maps.importProgressTooltip')}>{t('maps.importProgress')}</PillButton>
        ...
        {mapsLoading ? (
          ...
          <EmptyState emoji="🗺️" text={t('maps.noMapsYet')} />
        ) : (
            ...
            <aside className="w-full md:w-56 shrink-0 flex flex-row md:flex-col gap-1.5 overflow-x-auto md:overflow-x-visible md:max-h-[70vh] md:overflow-y-auto pb-1 md:pb-0">
              {visibleMaps.map((m, index) => {
                ...
                return (
                  <div key={m.id} className="relative shrink-0 md:shrink group">
                    ...
                    >
                      <div className={`flex items-center justify-between gap-2 ${isAdmin ? 'pr-5 pl-6' : ''}`}>
                        <span className="flex items-center gap-1 min-w-0">
                          {isMaxed && <span className={active ? 'text-gray-900' : 'text-yellow-400'} title={t('maps.maxReachedTooltip')}>★</span>}
                          {m.admin_only && <span className={active ? 'text-gray-900' : 'text-gray-500'} title={t('maps.adminOnlyTooltip')}>🔒</span>}
                    ...
                    {isAdmin && (
                      <ReorderButtons onUp={() => moveMap(index, -1)} onDown={() => moveMap(index, 1)} disableUp={index === 0} disableDown={index === visibleMaps.length - 1} />
                    )}
                    {isAdmin && (
                      <button onClick={e => { e.stopPropagation(); setEditingMap(m) }} title={t('maps.editMapTooltip')}
              ...
              {isAdmin && (
                <button onClick={() => setAddingMap(true)}
                  className="shrink-0 md:shrink text-left px-3 py-2 rounded-lg text-sm border border-dashed border-white/15 text-gray-400 hover:text-white hover:border-white/30 transition-colors">
                  {t('maps.addMap')}
                </button>
              )}
            </aside>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <PillButton onClick={() => setShowCollected(v => !v)}>{showCollected ? t('maps.hideCollected') : t('maps.showCollected')}</PillButton>
                  {markers.length > 0 && (
                    <PillButton onClick={() => setConfirmBulkMode(allCollectedOnMap ? 'deselect' : 'select')}>
              ...
              {markersLoading || !selectedMap ? (
                ...
                <>
                  <div
                    ref={mapViewportRef}
                    className={`relative w-full touch-none rounded-xl border border-white/10 bg-black/40 ${
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            {isPending && (
                              <span className="absolute -top-1 -right-1 text-xs drop-shadow" title={t('maps.pendingVisibilityTooltip')}>⏳</span>
                            )}
                          </button>
                          </div>
                        </div>
                  ...
                  {isTouchDevice && zoom > 1 && (
                    <button
                      ...
                      className="absolute top-2 left-2 px-2.5 py-1.5 rounded-lg text-xs font-semibold bg-black/70 hover:bg-black/80 border border-white/10 text-gray-200 backdrop-blur-sm"
                    >
                      {Math.round(zoom * 100)}% ⤾
                    </button>
                  )}
                  ...
                  <div
                    ...
                    onPointerCancel={e => e.stopPropagation()}
                  >
                    <button onClick={() => zoomTo(zoom + 1)} disabled={zoom >= MAX_ZOOM}
                      className="w-8 h-8 flex items-center justify-center rounded-lg text-base font-bold bg-black/70 hover:bg-black/80 disabled:opacity-40 border border-white/10 text-gray-200 backdrop-blur-sm">
                      +
                    </button>
                    <button onClick={() => zoomTo(zoom - 1)} disabled={zoom <= MIN_ZOOM}
                      className="w-8 h-8 flex items-center justify-center rounded-lg text-base font-bold bg-black/70 hover:bg-black/80 disabled:opacity-40 border border-white/10 text-gray-200 backdrop-blur-sm">
                      −
                    </button>
                  </div>
                  ...
                  {pendingReveal[selectedMap.id] && <MokokoRevealCountdown revealAt={pendingReveal[selectedMap.id]} />}
                </>
              )}
            </div>
          </div>
      ...
      {openMarker && <MarkerPanel marker={openMarker} onClose={() => setOpenMarker(null)} />}
      {showHelpers && <HallOfFameModal onClose={() => setShowHelpers(false)} />}
      <MokokoCompletionModal show={allMokokoCollected} />
      {confirmBulkMode && (
        <ConfirmBulkMarkModal mode={confirmBulkMode} onCancel={() => setConfirmBulkMode(null)} onConfirm={() => applyBulkMark(confirmBulkMode === 'select')} />
      )}
      {editingMap && (
        <EditMapModal map={editingMap} onClose={() => setEditingMap(null)} onUpdated={updated => setMaps(prev => prev.map(m => m.id === updated.id ? updated : m))} />
      )}
      {addingMap && (
        <AddMapModal nextSortOrder={Math.max(0, ...maps.map(m => m.sort_order)) + 10} onClose={() => setAddingMap(false)} onAdded={map => setMaps(prev => [...prev, map].sort((a, b) => a.sort_order - b.sort_order))} />
      )}
    </div>
  )
}
