...
export default function MapsH() {
  ...
  return (
    <div className="min-h-screen bg-[#14110d] text-white">
      ...
      {addingMap && (
        <AddMapModal defaultChapter={selectedMap ? mapChapterOf(selectedMap) : 1} nextSortOrder={Math.max(0, ...maps.map(m => m.sort_order)) + 10} onClose={() => setAddingMap(false)} onAdded={map => setMaps(prev => [...prev, map].sort((a, b) => a.sort_order - b.sort_order))} />
      )}
    </div>
  )
}
